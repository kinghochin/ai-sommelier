...
# Determine agent
AGENT_NAME = os.getenv("AGENT_NAME", "wine")
...
if AGENT_NAME == "wine":
    ...
    from config.law_config import AgentDeps, SYSTEM_PROMPT, MODEL_CONFIG
elif AGENT_NAME == "law-admin":
    from config.law_admin_config import AgentDeps, SYSTEM_PROMPT, MODEL_CONFIG
else:
    raise ValueError(f"Unknown AGENT_NAME: {AGENT_NAME}")

# At the top of your Streamlit app
st.set_page_config(
    page_title=MODEL_CONFIG[MODEL_CHOICE]["name"],
    page_icon=MODEL_CONFIG[MODEL_CHOICE]["emoji"],
    initial_sidebar_state="collapsed",
    layout="centered",
)

ENV = os.getenv("ENV", "init")
...
# Add sidebar hiding only if NOT development
if ENV != "development":
    css += """
    /* Hide sidebar */
    ...
    /* Hide sidebar toggle button (hamburger) */
...
if "user_agent" not in st.session_state:
    st.session_state.user_agent = st.query_params.get("user_agent", ["Unknown"])[0]
...
def initialize_rag():
    ...
    base_dir = os.path.expanduser(
        f"~/Documents/data/projects/ai/my-projects/{MODEL_CONFIG[MODEL_CHOICE]['id']}"
    )
...
@agent.tool
async def give_suggestion(ctx: RunContext[AgentDeps], query: str) -> float:
    """Suggestion based on a search query."""
    observation_relevant_chunks = get_rag_context(query, 3)
    observation_text = f"Observation: {observation_relevant_chunks}"
    conversation = []
    conversation.append(
        {"role": MODEL_CONFIG[MODEL_CHOICE]["name"], "content": f"{query}"}
    )
    conversation.append({"role": "Agent Wine", "content": observation_text})
    return conversation


st.image(MODEL_CONFIG[MODEL_CHOICE]["image"], width=128)
st.title(MODEL_CONFIG[MODEL_CHOICE]["title"])

# Update the chat history display
for message in st.session_state.messages:
    avatar = (
        MODEL_CONFIG[message.get("model")] if message["role"] == "assistant" else None
    )
...
if not st.session_state.messages:
    st.chat_message("assistant", avatar=MODEL_CONFIG[MODEL_CHOICE]["emoji"]).markdown(
        MODEL_CONFIG[MODEL_CHOICE]["help"]
    )
...
if prompt := st.chat_input("How can I help you today?"):
    ...
    try:
        ...
        with st.chat_message("assistant", avatar=MODEL_CONFIG[MODEL_CHOICE]["emoji"]):
